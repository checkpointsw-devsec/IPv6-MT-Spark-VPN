...
highlight_message '###################         adding spark3 ##################'
mgmt_cli add simple-gateway -s /home/admin/sid.txt\
		...
		ipv4-address "10.1.0.30" \
        firewall true \
		one-time-password "$otp" \
        trust-settings.initiation-phase 'now' \
        platform-portal-settings.portal-web-settings.main-url "https://[2a04:6447:900:100::30]/" \
        interfaces.1.name "$rugg_vlan100" \
        interfaces.1.ipv6-address "2a04:6447:900:100::30" \
        interfaces.1.ipv6-mask-length "64" \
		interfaces.1.ipv4-address "10.1.0.30" \
        ...
        interfaces.2.ipv6-address "2a04:6447:900:101::30" \
        interfaces.2.ipv6-mask-length "64" \
		interfaces.2.ipv4-address "10.1.1.30" \
        ...
        interfaces.3.ipv6-address "2a04:6447:900:102::30" \
        interfaces.3.ipv6-mask-length "64" \
		interfaces.3.ipv4-address "10.1.2.30" \
        ...
        interfaces.4.ipv6-address "2a04:6447:900:3100::1" \
        interfaces.4.ipv6-mask-length "64" \
		interfaces.4.ipv4-address "10.31.0.30" \
        ...
        interfaces.5.ipv6-address "2a04:6447:900:3101::1" \
        interfaces.5.ipv6-mask-length "64" \
		interfaces.5.ipv4-address "10.31.1.30" \
        ...
        interfaces.6.ipv6-address "2a04:6447:900:3102::1" \
        interfaces.6.ipv6-mask-length "64" \
		interfaces.6.ipv4-address "10.31.2.30" \
...
highlight_message '############### enable vpn blade on spark3       ###########'
...
highlight_message '############### add ELS interfaces  spark3       ###########'
mgmt_cli set-simple-gateway -s /home/admin/sid.txt\
         name "vpn-gw-3" \
		 ipv6-address "2a04:6447:900:100::30" \
		 allow-smb true \
		 vpn-settings.vpn-domain-type "manual" \
		 vpn-settings.vpn-domain "Spark3_EncDom46" \
...
myUID0=$(mgmt_cli show-generic-objects -s /home/admin/sid.txt name "vpn-gw-3" -f json | jq '.objects[0].uid')
mgmt_cli set-generic-object -s /home/admin/sid.txt uid $myUID0 securityBladesTopologyMode "TOPOLOGY_TABLE"        

myUID1=$(mgmt_cli show-generic-objects -s /home/admin/sid.txt name "vpn-gw-3" -f json | jq '.objects[1].uid')
mgmt_cli set-generic-object -s /home/admin/sid.txt uid $myUID1 securityBladesTopologyMode "TOPOLOGY_TABLE"        

cprid_util -server 2a04:6447:900:100::30 -verbose rexec -rcmd /bin/bash -c "LOGNAME=admin bashUser on"
